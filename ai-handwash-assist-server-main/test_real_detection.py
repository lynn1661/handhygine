...
def test_real_handwash_detection():
    print("🧪 开始测试真实AI洗手检测服务...")
    ...
    try:
        ...
        # 初始化洗手检测服务（使用真实AI模型）
        print("🚀 初始化洗手检测服务（真实AI模型）...")
        handwash_service = HandwashDetectionService(db_service, use_real_model=True)
        ...
        for step in range(1, 8):
            result = handwash_service.analyze_handwash(
                data=test_data,
                step=step,
                request_id=f'real_ai_test_step_{step}'
            )
...
if __name__ == '__main__':
    success = test_real_handwash_detection()
    if success:
        print("🎉 真实AI洗手检测服务测试成功！")
    else:
        print("💥 真实AI洗手检测服务测试失败！")
